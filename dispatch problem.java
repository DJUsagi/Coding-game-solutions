...
class Player {
    ...
    public static void main(String args[]) 
    {
        ...
        System.err.println(targetWeight);
        System.err.println(totalVol);
        ...
        while(done == false)
        {
            ...
            for(int x = 0; x < NUM_TRUCKS; x++) // for each truck starting at the lightest
            {
                ...
                for(int y = 0; y < temp1.getNumBoxes(); y++) //for every box on that truck
                {
                    ...
                    if(temp1Delta < 0.5) continue; //if we have sorted this then don't worry
            ...
            //Arrays.sort(trucks); // lightest to heaviest
            double h = 0;
            double l = 100000;
            int p = 0;
            for(Truck el: trucks)
...
class Truck implements Comparable<Truck>
{
    ...
    public boolean addBox(Box in) //returns false if we can add this box.
    {
        ...
        //System.err.println("Truck " + truckIndex + " now weighs " + totalWeight);
        return true;
    }
